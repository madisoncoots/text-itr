...
X_offense <- c("StatenIsland", "Bronx", "Brooklyn", "Queens", "Park", "Alcohol", "Marijuana",
               "DisorderlyAcvtive", "DisorderlyPassive", "Disorderly", "Bike", "Noise",
               "MotorVehicle", "Urination")
# individual-vased covariates
X_perp <- c("Female", "Age", "PastFTA", "NbPastFTA", "NbSummons",  "PastSummons", 
            "MedianIncZip", "PctBlackZip", "PctHispZip", "PctBelowPovZip", "DOW")
# all covariates
X_full <- c(X_offense, X_other)

# fit forests
forest_bin_full<- causal_forest(df[X_full], as.vector(df$success), as.vector(df$Treatment))
forest_bin_offense<- causal_forest(df[X_offense], as.vector(df$success), as.vector(df$Treatment))
forest_bin_perp<- causal_forest(df[X_perp], as.vector(df$success), as.vector(df$Treatment))
