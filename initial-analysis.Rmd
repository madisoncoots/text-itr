...
appearance_rate_by_demo <- function(data, treatment, demo = "All", threshold = 0.5, places = 3) {
  if (demo == "All")
    tmp <- tmp <- data[which(data[treatment] == 1),]
  else
...
res <- matrix(data = 0, nrow = length(demos), ncol = 3)
for (d in  1:length(demos)) {
  res[d, 1] <- appearance_rate_by_demo(df2, "planmaking", demo = demos[d])
  res[d, 2] <- appearance_rate_by_demo(df2, "consequences", demo = demos[d])
  res[d, 3] <- appearance_rate_by_demo(df2, "combination", demo = demos[d])
}
```
\begin{tabular}{l r | c c c}
Demo. & Group & Planmaking & Consequences & Combination \\ \hline
Gender & Male & `r res[1,1]` & `r res[1,2]` & `r res[1,3]` \\
 & Female & `r res[2,1]` & `r res[2,2]` & `r res[2,3]`  \\ \hline
Race & Black & `r res[3,1]` & `r res[3,2]` & `r res[3,3]`  \\
 & Hispanic & `r res[4,1]` & `r res[5,2]` & `r res[5,3]`  \\
 & Black or Hispanic & `r res[5,1]` & `r res[5,2]` & `r res[5,3]`  \\ \hline
Poverty & Below Poverty Line & `r res[6,1]` & `r res[6,2]` & `r res[6,3]` 
\end{tabular}
